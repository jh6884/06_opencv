...
while cap.isOpened():       
    ...
    if img1 is None:  # 등록된 이미지 없음, 카메라 바이패스
        ...
        img2 = frame
        # [step 1]
        gray1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY) # 참조 이미지(ROI)
        gray2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY) # 카메라 입력
        
        # [step 2]
        # 키포인트와 디스크립터 추출
        # kp: 키포인트keypoint 특징점의 위치 정보
        # desc: 특징점의 특성을 숫자로 표현
        kp1, desc1 = detector.detectAndCompute(gray1, None) # 참조 이미지의 특징점
        kp2, desc2 = detector.detectAndCompute(img2, None) # 카메라 입력에서 특징점
